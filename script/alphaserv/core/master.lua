...
alpha.settings.init_setting("register_server", true, "bool", "register server in the serverlist you choose.")
alpha.settings.init_setting("register_timeout", 36000, "number", "time between register attempts.")
--alpha.settings.init_setting("master_servers", "{'sauerbraten.org:1234'}", "table{string}", "the masterservers to register to.\nusage: '\"{ 'address:port' }\"', without single quotes (')")
alpha.settings.init_setting("master_server_workaround", "sauerbraten.org:28787", "string", "the master server to register to.\n usage: \"host:port\"")

function alpha.master.register_server(hostname, port)

	if not alpha.settings:get("register_server") then return false, "registering server is not enabled in the config file" end
...
function alpha.master.init()
	if not alpha.settings:get("register_server") then return false, "registering server is not enabled in the config file" end
	
	local function loop()
		--[[
		for i, string_ in pairs(assert(alpha.settings:get("master_servers"))) do
			string.gsub(string_, "(.-)[:]([0-9]*)", function(ip, port)
				alpha.master.register_server(tostring(ip), tonumber(port))
			end)
		end]]
		string.gsub(alpha.settings:get("master_server_workaround"), "(.-)[:]([0-9]*)", function(ip, port)
			alpha.master.register_server(tostring(ip), tonumber(port))
		end)
	end
	loop()
--	server.interval(alpha.settings:get("master_time"), loop)
end

server.event_handler("config_loaded", alpha.master.init)

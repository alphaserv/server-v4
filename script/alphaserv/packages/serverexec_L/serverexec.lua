...
local function read_expression(existing_code, discard_time_limit)
	...
	file_stream:async_read_some	(READ_SIZE, function(new_code, error_message)
		...
		if not new_code then
			alpha.log.message_to("serverexec read error: %s", "serverexec", error_message)
		end
		
		if discard_time_limit and os.time() > discard_time_limit then
   			alpha.log.message_to("error in serverexec: %s", "serverexec", "discarding old incomplete code")
			existing_code = ""
		end
		...
		if error_message then
			...
			local code = "\n<!-- START CODE -->\n" .. code .. "<!-- END CODE -->"
   			alpha.log.message_to("error in serverexec: %s \ncode: %q", "serverexec", error_message, code)
		end

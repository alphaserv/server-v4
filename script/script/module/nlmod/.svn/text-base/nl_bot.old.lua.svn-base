...
irc.irclog = io.open("log/irc.log","a+")

function irc.log_irc(msg)
--[[
			Schreibt in log/irc.log
]]
	assert(msg ~= nil)
...
local function print_debug(msg)
    if debug_mode then
        irc.log_irc(string.format("IRC DEBUG %s", msg))
    end
end
...
function irc:processData(client, data)
	...
	if data.find(data,"PING") then
		...
		if debug_mode then irc.log_irc("DEBUG [processData:PING] : "..pong) end
	...
	if data.find(data,"NOTICE") then
		server.sleep(10000, function()
						--irc.client:async_send("OPER NL4-Bot superdillgurke\n", noop)
...
function irc.processCommand(nick, channel, command)
    ...
    local function handleUnknownCommand()
        ...
        irc:toChannel(string.format("[processCommand] : %s <%s> %s", channel or "<error>", nick or "<error>", commandName or "<error>"))
    end
...
function irc:playerList()
	...
	for player in server.gplayers('all') do
		...
		if counter_total == nil then counter_total = 0 end
		counter = counter + 1
		counter_total = counter_total + 1
		output = output .. string.format("\00312%s\003(%s) ", player:displayname(), player.cn)
		if counter == 5 then
			irc:toChannel("\0034WHO\003 is "..output)
			counter = 0
			output = ""
		end
	end
	...
	if counter == nil then 
		irc:toChannel("\0034WHO\003 Apparently no one is connected.") 
	else
		if counter < 5 and counter > 0 then
			irc:toChannel("\0034WHO\003 is "..output)
		end
		irc:toChannel(string.format("\0034-/WHO/-\003 %i players connected.", counter_total))
	end

end

function irc.spamTest()

	my_list = {"Zeile 1","Zeile 2","Zeile 3","Zeile 4","Zeile 5","Zeile 6","Zeile 7","Zeile 8","Zeile 9"}
    
	for i, zeile in pairs(my_list) do
		irc:toChannel(string.format("\0034%s\003 Dies ist ein beschissener Spam-Test ...", zeile))
	end
...
function irc:status()
  ...
  irc:toChannel("\0034STATUS\003 I am working on this ;)")
...
irc.commands = {
    ...
    ['who']			= irc.playerList,
    ['blah']        = function() irc.spamTest() end,
    ['status']			= irc.status
}

function irc_say(myTXT)
	irc:toChannel(myTXT)
end

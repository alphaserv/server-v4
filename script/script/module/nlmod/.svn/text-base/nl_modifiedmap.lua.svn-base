--[[
	script/module/nl_mod/nl_modifiedmap.lua
	...
	Funktionen:
		Erkennen von veränderten Maps und solche Spieler in den Spectator schicken
...
modifiedmap = {}
modifiedmap.module_name = "MODIFIED MAP"
modifiedmap.players = {}

modifiedmap.maps = {}
function modifiedmap.load ()
	local maps = db.select( "nl_mapcrc", { "map", "crc" }, "length(map) > 0" )
	for i,map in pairs(maps) do
		-- irc_say( string.format("%s=%s", map.map, map.crc) )
		modifiedmap.maps[map.map] = { crc = map.crc }
		--table.insert(modifiedmap.maps["test"], { map.crc } )
	end
end
modifiedmap.load()
...
server.event_handler("mapcrc", function(cn, map, crc)
	...
	local map_info = modifiedmap.maps[map]
	if not map_info or not map_info.crc then
		if server.nmgrpid == "ml" then
			messages.error(cn, {cn}, "WARNING", "You are using a modified map!")
			messages.warning(cn, players.admins(), "WARNING", server.player_displayname(cn) .. " (" .. cn .. ") is using a modified map!")
			spectator.fspec(cn, "MODIFIEDMAP", modifiedmap.module_name)
			table.insert(modifiedmap.players, cn)
		end
		return
	end
	if crypto.tigersum(tostring(crc)) ~= map_info.crc then
		if server.nmgrpid == "ml" then
			messages.error(cn, {cn}, "WARNING", string.format("%s, you are using a modified map!", server.player_displayname(cn)))
			messages.warning(cn, players.admins(), string.format("%s, you are using a modified map!", server.player_displayname(cn)))
			messages.error(cn, {cn}, "WARNING", "Download the maps here: www.maplounge.net")
			messages.warning(cn, players.admins(), "Download the maps here: www.maplounge.net")
			spectator.fspec(cn, "MODIFIEDMAP", modifiedmap.module_name)
			table.insert(modifiedmap.players, cn)
		else
			messages.error(cn, {cn}, "CHEATER", "You are using a modified map!")
			messages.warning(cn, players.admins(), "CHEATER", server.player_displayname(cn) .. " (" .. cn .. ") is using a modified map!")
			spectator.fspec(cn, "MODIFIEDMAP", modifiedmap.module_name)
			table.insert(modifiedmap.players, cn)
		end
		return
...
server.event_handler("intermission", function()
	for _, cn in pairs(modifiedmap.players) do

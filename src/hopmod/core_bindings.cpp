...
void bind_core_functions(lua_State * L, int T)
{
	...
	bind_function(L, T, "fake_msg", server::load_map);
	bind_function(L, T, "fake_msg", server::fake_msg);
	bind_function(L, T, "fake_text", server::fake_text);
	bind_function(L, T, "no_spawn", server::no_spawn);
	bind_function(L, T, "fake_disconnect", server::fake_disconnect);
	bind_function(L, T, "do_spawn_player", server::do_spawn_player);
	bind_function(L, T, "editvar", server::editvar);
	bind_function(L, T, "fake_connect", server::fake_connect);
	bind_function(L, T, "send_sound", server::send_sound);
	bind_function(L, T, "damage_player", server::damage_player);
	
	bind_function(L, T, "send_gamestate", server::send_gamestate);
	bind_function(L, T, "set_gamestate", server::set_gamestate);
	bind_function(L, T, "set_ammo", server::set_ammo);
}
...
{	
	...
	#define def_const(a) bind_function(L, T, #a, get_constant< a >);
	...
	def_const(S_RLFIRE)
	def_const(S_RLHIT)
	def_const(S_WEAPLOAD)
	def_const(S_ITEMAMMO)
	def_const(S_ITEMHEALTH)
	
	def_const(S_ITEMARMOUR)
	def_const(S_ITEMPUP)
	def_const(S_ITEMSPAWN)
	def_const(S_TELEPORT)
	def_const(S_NOAMMO)
	def_const(S_PUPOUT)
	...
	def_const(S_PIGR1)
	def_const(S_ICEBALL)
	def_const(S_SLIMEBALL)
	def_const(S_JUMPPAD)
	def_const(S_PISTOL)

	def_const(S_V_BASECAP)
	def_const(S_V_BASELOST)
	def_const(S_V_FIGHT)
	def_const(S_V_BOOST)
	def_const(S_V_BOOST10)
	def_const(S_V_QUAD)
	def_const(S_V_QUAD10)
	def_const(S_V_RESPAWNPOINT)
	
	def_const(S_FLAGPICKUP)
	def_const(S_FLAGDROP)
	def_const(S_FLAGRETURN)
	def_const(S_FLAGSCORE)
	def_const(S_FLAGRESET)
	
	def_const(S_BURN)
	def_const(S_CHAINSAW_ATTACK)
	def_const(S_CHAINSAW_IDLE)

...
{
	sendf(cn_to, 1, "riis", N_SAYTEAM, cn_from, text);
}
void fake_text(int cn_from, int cn_to, const char * text)
{
	sendf(cn_to, 1, "riis", N_TEXT, cn_from, text);
}
...
{
	//make shure that it's a valid cn
	get_ci(cn);
...
void send_sound(int cn, int num)
{
	...
	sendf(cn, 1, "ri5", N_CLIENT, cn, 2, N_SOUND, num);
}
...
int damage_player (int cn, int damage)
{
	...
	damage = ci->state.dodamage(damage);
	
	sendf(-1, 1, "ri6", N_DAMAGE, cn, cn, damage, ci->state.armour, ci->state.health); 
...
void send_gamestate(int cn)
{
	...
	putint(p, ci->clientnum);
	putint(p, ci->state.lifesequence);
	putint(p, ci->state.health);
	putint(p, ci->state.maxhealth);
	putint(p, ci->state.armour);
	putint(p, ci->state.armourtype);
	putint(p, ci->state.gunselect);

	loopi(GUN_PISTOL-GUN_SG+1)
		putint(p, ci->state.ammo[GUN_SG+i]);
	
	sendpacket(-1, 1, p.finalize());
}

void set_gamestate(int cn, int health, int maxhealth, int armour, int armourtype, int gunselect)
{
	clientinfo *ci = get_ci(cn);
